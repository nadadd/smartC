...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'dateAjout'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      dateAjout: product.dateAjout?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      dateAjout: restProduct.dateAjout ? dayjs(restProduct.dateAjout) : undefined,
    };
  }
